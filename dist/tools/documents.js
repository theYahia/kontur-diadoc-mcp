...
export async function handleListOrganizations(params) {
    const query = {};
    if (params.inn)
        query.innKpp = params.kpp ? `${params.inn}-${params.kpp}` : params.inn;
...
export async function handleGetOrganization(params) {
...
export async function handleListDocuments(params) {
    const query = { boxId: params.box_id };
    if (params.filter_category)
        query.filterCategory = params.filter_category;
    if (params.timestamped_from)
        query.timestampedFrom = params.timestamped_from;
    if (params.timestamped_to)
        query.timestampedTo = params.timestamped_to;
...
export async function handleSendDocument(params) {
    const body = {
...
export async function handleSignDocument(params) {
    const body = {
        BoxId: params.box_id,
        MessageId: params.message_id,
        RecipientTitles: [{
...
export async function handleListCounterparties(params) {
